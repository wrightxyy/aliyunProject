window.SINGLE_TAB = "  ";
window.ImgCollapsed = "fa fa-plus-square-o";
window.ImgExpanded = "fa fa-minus-square-o";
...
function Process(json, cid) {
    ...
    try {
        ...
        $id(cid).innerHTML = "<PRE class='CodeContainer'>" + html + "</PRE>";
    } catch (e) {
        $id(cid).innerHTML = "JSON数据格式不正确:\n" + e.message;
    }
}
...
function ProcessObject(obj, indent, addComma, isArray, isPropertyContent) {
    ...
    if (IsArray(obj)) {
        if (obj.length == 0) {
            ...
            clpsHtml = window.IsCollapsible ? "<span><i class=\"" + window.ImgExpanded + "\" onClick=\"ExpImgClicked(this)\"></i></span><span class='collapsible'>" : "";
...
function MakeContentVisible(element, visible) {
    ...
    if (!!img.tagName && img.tagName.toLowerCase() == "i") {
        ...
        element.previousSibling.firstChild.class = visible ? 'fa fa-plus-square-o' : 'fa fa-minus-square-o';
...
function ExpImgClicked(img) {
    ...
    var css = window.ImgCollapsed;

    if (container.style.display == "none") {

        disp = "inline";

        css = window.ImgExpanded;
